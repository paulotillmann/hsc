...
import { 
  ...
  SolicitacaoCadastro,
  HistoricoSolicitacaoCadastro,
  fetchSolicitacoesCadastro,
  fetchIndicadoresCadastro,
  iniciarAnaliseCadastro,
  aprovarSolicitacaoCadastro,
  rejeitarSolicitacaoCadastro,
  atualizarSolicitacaoCadastroCompleta,
  fetchHistoricoCadastro,
  criarSolicitacaoCadastroTeste
} from '../../services/prontuarioService';


export default function GestaoProntuarios() {
  const { profile } = useAuth();
  const [abaPrincipal, setAbaPrincipal] = useState<'prontuarios' | 'acessos'>('prontuarios');
  ...
  // Cadastro/Acesso States
  const [solicitacoesCadastro, setSolicitacoesCadastro] = useState<SolicitacaoCadastro[]>([]);
  const [indicadoresCadastro, setIndicadoresCadastro] = useState<IndicadoresProntuario>({
    pendentes: 0,
    emAnalise: 0,
    aprovadas: 0,
    rejeitadas: 0,
  ...
  const [selectedCadastro, setSelectedCadastro] = useState<SolicitacaoCadastro | null>(null);
  const [historico, setHistorico] = useState<HistoricoSolicitacao[]>([]);
  const [historicoCadastro, setHistoricoCadastro] = useState<HistoricoSolicitacaoCadastro[]>([]);
  ...
  const handleCriarSolicitacaoTeste = async () => {
    ...
    try {
      if (abaPrincipal === 'prontuarios') {
        ...
        await criarSolicitacaoCadastroTeste();
        mostrarNotificacao('sucesso', 'Solicitação de cadastro de teste criada com sucesso!');
      }
      carregarDados();
    } catch (err: any) {
      console.error(err);
  ...
  const carregarDados = async () => {
    ...
    try {
      if (abaPrincipal === 'prontuarios') {
        ...
        const [list, inds] = await Promise.all([
          fetchSolicitacoesCadastro(busca, statusFiltro),
          fetchIndicadoresCadastro()
        ]);
        setSolicitacoesCadastro(list);
        setIndicadoresCadastro(inds);
      }
    } catch (err) {
  ...
  useEffect(() => {
    carregarDados();
  }, [busca, statusFiltro, abaPrincipal]);
  ...
  const handleOpenDetails = async (sol: SolicitacaoProntuario) => {
    setLoadingAction(true);
    try {
      const hist = await fetchHistorico(sol.id);
      setHistorico(hist);
      setSelectedSolicitacao(sol);
      setSelectedCadastro(null);
      setNovoStatusSelecionado(sol.status);
      setAbaAtiva('dados');
      setModoAcao('visualizar');
      setArquivoSelecionado(null);
      setJustificativa(sol.justificativa_rejeicao || '');
      setErroAcao('');
      setIsModalOpen(true);
    } catch (err) {
  ...
  const handleOpenDetailsCadastro = async (sol: SolicitacaoCadastro) => {
    setLoadingAction(true);
    try {
      const hist = await fetchHistoricoCadastro(sol.id);
      setHistoricoCadastro(hist);
      setSelectedCadastro(sol);
      ...
      console.error('Erro ao abrir detalhes do cadastro:', err);
      mostrarNotificacao('erro', 'Falha ao buscar histórico da solicitação.');
    } finally {
      setLoadingAction(false);
    }
  ...
  const handleIniciarAnalise = async (sol: SolicitacaoProntuario) => {
    setLoadingAction(true);
    try {
      const userName = profile?.full_name || profile?.email || 'Gestor';
      const userId = profile?.id || null;
      await iniciarAnalise(sol.id, userName, userId);
      
      mostrarNotificacao('sucesso', `Análise da solicitação #${String(sol.numero_solicitacao).padStart(4, '0')} iniciada.`);
  ...
  const handleIniciarAnaliseCadastro = async (sol: SolicitacaoCadastro) => {
    setLoadingAction(true);
    try {
      const userName = profile?.full_name || profile?.email || 'Gestor';
      const userId = profile?.id || null;
      await iniciarAnaliseCadastro(sol.id, userName, userId);
      
      mostrarNotificacao('sucesso', `Análise da solicitação de acesso #${String(sol.id).padStart(4, '0')} iniciada.`);
      setIsModalOpen(false);
      carregarDados();
    } catch (err) {
      console.error(err);
      mostrarNotificacao('erro', 'Erro ao iniciar análise de cadastro.');
    } finally {
      setLoadingAction(false);
    }
  };
  ...
  const handleSalvarAlteracoesGestor = async () => {
    if (abaPrincipal === 'prontuarios') {
      ...
      try {
        ...
        await atualizarSolicitacaoCadastroCompleta(
          selectedCadastro.id,
          novoStatusSelecionado,
          userName,
          userId,
  ...
  const currentIndicadores = abaPrincipal === 'prontuarios' ? indicadores : indicadoresCadastro;
  ...
  return (
    <div className="flex flex-1 flex-col gap-8 p-6 md:p-8 animate-in fade-in duration-500 bg-background text-foreground pb-16">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            {abaPrincipal === 'prontuarios' 
              ? 'Receba, analise, aprove ou rejeite solicitações de prontuários com entrega digital ou controle físico.' 
              : 'Gerencie e verifique as solicitações de acesso e pré-cadastro de pacientes que ainda não possuem registro no sistema.'}
          </p>
        </div>
      ...
      {/* TABS PRINCIPAIS */}
      <div className="flex border-b border-border bg-card rounded-xl overflow-hidden p-1 gap-1 self-start shadow-xs">
        <button
          onClick={() => setAbaPrincipal('prontuarios')}
          className={`px-5 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-2 cursor-pointer ${
            abaPrincipal === 'prontuarios' 
              ? 'bg-[#8a1515] text-white dark:bg-[#f43f5e]' 
              : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
          }`}
        >
          <FileText className="w-4 h-4" />
          Solicitações de Prontuários
        </button>
        <button
          onClick={() => setAbaPrincipal('acessos')}
          className={`px-5 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-2 cursor-pointer ${
            abaPrincipal === 'acessos' 
              ? 'bg-[#8a1515] text-white dark:bg-[#f43f5e]' 
              : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
          }`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        <VisaoGeralCard 
          title="Pendentes" 
          value={currentIndicadores.pendentes} 
          icon={Clock} 
        ...
        <VisaoGeralCard 
          title="Em Análise" 
          value={currentIndicadores.emAnalise} 
          icon={RotateCw} 
        ...
        <VisaoGeralCard 
          title="Aprovadas" 
          value={currentIndicadores.aprovadas} 
          icon={CheckCircle2} 
          subtext={abaPrincipal === 'prontuarios' ? "✓ Aguardando anexo PDF" : "✓ Cadastro aprovado"}
          subtextColorClass="text-blue-600 dark:text-blue-400"
          isLoading={loading}
        />
        <VisaoGeralCard 
          title="Rejeitadas" 
          value={currentIndicadores.rejeitadas} 
          icon={XCircle} 
        ...
        />
        <VisaoGeralCard 
          title={abaPrincipal === 'prontuarios' ? "Documentos Entregues" : "Acessos Concluídos"} 
          value={currentIndicadores.disponibilizados} 
          icon={FileCheck} 
          subtext={abaPrincipal === 'prontuarios' ? "📂 PDFs disponibilizados" : "✅ Cadastros concluídos"}
      ...
      <div className="bg-card p-5 rounded-2xl border border-border flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
        <div className="relative w-full md:w-96">
          ...
          <input
            type="text"
            placeholder={abaPrincipal === 'prontuarios' ? "Buscar por Paciente, CPF ou Número..." : "Buscar por Paciente, CPF ou Código..."}
            value={busca}
        ...
        <div className="overflow-x-auto">
          {abaPrincipal === 'prontuarios' ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border bg-slate-50/50 dark:bg-slate-900/20">
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground w-28">Código</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">Paciente</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground w-36">CPF</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground w-40">Contato</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">E-mail</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground w-56">Status</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground">Analista</th>
                  <th className="p-4.5 text-xs font-bold uppercase tracking-wider text-muted-foreground text-right w-36">Ações</th>
                </tr>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="p-12 text-center text-muted-foreground">
                      <div className="flex flex-col items-center justify-center gap-3">
                        <FileMinus className="w-10 h-10 text-muted-foreground/60" />
                        <span className="text-sm">Nenhuma solicitação de acesso/cadastro encontrada.</span>
                      </div>
                    </td>
                  </tr>
                ) : (
                  solicitacoesCadastro.map((sol) => (
                    <tr 
                      key={sol.id} 
                      onClick={() => handleOpenDetailsCadastro(sol)}
                      className="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition-colors cursor-pointer"
                    >
                      <td className="p-4.5 font-mono text-sm font-semibold text-[#8a1515] dark:text-[#f43f5e]">
                        #{String(sol.id).padStart(4, '0')}
                      </td>
                      <td className="p-4.5 font-medium text-foreground">
                        {sol.paciente_nome}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {sol.status === 'Pendente' && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleIniciarAnaliseCadastro(sol);
                              }}
                              title="Iniciar Análise"
                              disabled={loadingAction}
                              className="p-2 hover:bg-amber-50 text-amber-600 hover:text-amber-700 rounded-lg transition-colors border border-transparent hover:border-amber-200 cursor-pointer disabled:opacity-50"
      ...
      {/* MODAL DETALHAMENTO & AÇÃO (PRONTUÁRIOS) */}
      ...
      {/* MODAL DETALHAMENTO & AÇÃO (SOLICITAÇÕES DE CADASTRO / ACESSO) */}
      {isModalOpen && selectedCadastro && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-card border border-border w-full max-w-3xl rounded-2xl shadow-xl flex flex-col max-h-[85vh] overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-b border-border flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/20">
              <div className="flex items-center gap-3">
                <span className="font-mono text-lg font-bold text-[#8a1515] dark:text-[#f43f5e]">
                  #{String(selectedCadastro.id).padStart(4, '0')}
                </span>
                <h2 className="text-lg font-bold text-foreground truncate max-w-sm sm:max-w-md">
                  {selectedCadastro.paciente_nome}
                </h2>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {abaAtiva === 'dados' ? (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3.5 bg-slate-50/50 dark:bg-slate-900/10 p-4.5 rounded-xl border border-border/60">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                        <User className="w-3.5 h-3.5 text-[#8a1515] dark:text-[#f43f5e]" />
                        Dados do Paciente (Externo)
                      </h3>
                      <div className="space-y-2">
                        <div>
                          <span className="text-[11px] text-muted-foreground uppercase block font-semibold">Nome Completo</span>
                          <span className="text-sm font-medium text-foreground">{selectedCadastro.paciente_nome}</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <span className="text-[11px] text-muted-foreground uppercase block font-semibold">CPF</span>
                            <span className="text-sm font-medium text-foreground">{selectedCadastro.paciente_cpf}</span>
                          </div>
                          <div>
                            <span className="text-[11px] text-muted-foreground uppercase block font-semibold">Nascimento</span>
                            <span className="text-sm font-medium text-foreground">
                              {new Date(selectedCadastro.paciente_data_nascimento).toLocaleDateString('pt-BR')}
                            </span>
                          </div>
                        </div>
                        <div>
                          ...
                          <span className="text-sm font-medium text-foreground flex items-center gap-1">
                            <Phone className="w-3 h-3 text-muted-foreground" />
                            {selectedCadastro.paciente_contato}
                          </span>
                        </div>
                        <div>
                          <span className="text-[11px] text-muted-foreground uppercase block font-semibold">E-mail</span>
                          <span className="text-sm font-medium text-foreground">{selectedCadastro.paciente_email}</span>
                        </div>
                      </div>
                    </div>

                    {/* Detalhes do Pedido de Acesso */}
                    <div className="space-y-3.5 bg-slate-50/50 dark:bg-slate-900/10 p-4.5 rounded-xl border border-border/60">
                      <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                        <FileText className="w-3.5 h-3.5 text-[#8a1515] dark:text-[#f43f5e]" />
                        Detalhes do Pedido de Acesso
                      ...
                      <div className="space-y-2">
                        <div>
                          <span className="text-[11px] text-muted-foreground uppercase block font-semibold">Data Abertura</span>
                          <span className="text-sm font-medium text-foreground">
                            {new Date(selectedCadastro.data_solicitacao).toLocaleString('pt-BR')}
                          </span>
                        </div>
                        <div>
                          <span className="text-[11px] text-muted-foreground uppercase block font-semibold">Objetivo</span>
                          <span className="text-xs font-medium text-foreground block bg-background p-2 rounded-lg border border-border/40">
                            Solicitação de pré-cadastro e acesso ao sistema devido à falta de registro prévio no Tasy.
                          </span>
                        </div>
                        {selectedCadastro.justificativa_rejeicao && (
                          <div>
                            <span className="text-[11px] text-muted-foreground uppercase block font-semibold text-red-500">Motivo da Rejeição</span>
                            <span className="text-xs font-semibold text-red-600 block bg-red-50/50 p-2 rounded-lg border border-red-200">
                              {selectedCadastro.justificativa_rejeicao}
                            </span>
                          </div>
                        )}
                  ...
                  <div className="pt-6 border-t border-border space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        >
                          <option value="Pendente">Pendente</option>
                          <option value="Em Análise">Em Análise</option>
                          <option value="Aprovado">Aprovado (Cadastrado)</option>
                          <option value="Rejeitado">Rejeitado</option>
                        </select>
                      </div>
                      ...
                      <div className="flex flex-col justify-center p-3 bg-slate-50/50 dark:bg-slate-900/10 rounded-xl border border-border/40 text-xs">
                        <span className="text-[10px] text-muted-foreground uppercase font-bold">Status Atual</span>
                        <div className="mt-1 flex items-center gap-2">
                          <span className={getStatusBadge(selectedCadastro.status)}>
                            {getStatusIcon(selectedCadastro.status)}
                    ...
                    {(novoStatusSelecionado !== selectedCadastro.status) && (
                      <div className="flex gap-2 justify-end pt-2 animate-in fade-in duration-200">
                        <button
                          onClick={() => {
                            setNovoStatusSelecionado(selectedCadastro.status);
                            setJustificativa(selectedCadastro.justificativa_rejeicao || '');
                            setErroAcao('');
                          }}
                          className="px-4 py-2 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all"
                        >
                ...
                <div className="space-y-6 py-2">
                  {historicoCadastro.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-6">Nenhum evento registrado no histórico.</p>
                  ) : (
                    <div className="relative border-l-2 border-slate-200 dark:border-slate-800 ml-4.5 space-y-6">
                      {historicoCadastro.map((hist, idx) => (
                        <div key={hist.id || idx} className="relative pl-7">
