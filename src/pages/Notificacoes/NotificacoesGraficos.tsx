  ...
  const { doencasSetorData, setoresUnicos } = useMemo(() => {
    ...
    const DataArray = Object.entries(dataMap)
      .map(([name, counts]) => {
        ...
        rowData.total_percent = ((counts.total / totalGeral) * 100).toFixed(1).replace('.', ',') + '%';
        return rowData;
      })
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (img.complete && img.naturalWidth > 0) {
        logoHeight = 35; // Mesma altura usada no relatório de listagem
        logoWidth = (img.naturalWidth / img.naturalHeight) * logoHeight;
        startY += logoHeight + 20; // Espaçamento entre a logo e o print da div
      }

      const pdfWidth = elementWidth + (margin * 2);
      const pdfHeight = startY + elementHeight + margin;

      // Criando PDF tamanho customizado em pixels para garantir as proporções exatas da tela do hospedeiro
      ...
      // Desenha a logo primeiramente (se encontrou com sucesso)
      if (logoHeight > 0) {
        customPdf.addImage(img, 'PNG', margin, margin, logoWidth, logoHeight);
      }

      // Desenha o screenshot do gráfico logo abaixo da logo
      customPdf.addImage(imgData, 'PNG', margin, startY, elementWidth, elementHeight);
                      ...
                      >
                        <LabelList
                          ...
                          content={(props: any) => {
                            ...
                            if (!payload || payload.lastValidSetor !== setor) return null;
                            return (
                              <text
                              ...
                              >
                                {payload.total} ({payload.total_percent})
                              </text>
                            );
                          }}
                        />
