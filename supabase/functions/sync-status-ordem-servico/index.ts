...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (reqBody && reqBody.action === 'inspect_os' && reqBody.nr_sequencia) {
      const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);
      const targetSeq = Number(reqBody.nr_sequencia);
      const { data: osData } = await supabase.from('ordem_servico').select('*').eq('nr_sequencia', targetSeq);
      const { data: histData } = await supabase.from('historico_ordem_servico').select('*').eq('nr_sequencia', targetSeq).order('dt_historico', { ascending: false }).order('created_at', { ascending: false });
      return new Response(JSON.stringify({ success: true, osData, histData }), { status: 200, headers: corsHeaders });
    }
    ...
    // 3. Buscar OS cadastradas no banco de dados do Supabase (todas de 2026)
    console.log('[Sync Status OS] Buscando OS cadastradas no banco de dados...');
    ...
    // 4. Mapear e identificar alterações de status ou novo histórico em OSs existentes
    const recordsToUpdateMap = new Map<number, DBOrdemServico>();
    const deletedSequences = new Set<number>();
    const validItemsMap = new Map<number, N8NOrdemServico>();
    ...
    for (const item of rawData) {
      ...
      validItemsMap.set(nrSeq, item);

      const existing = existingMap.get(nrSeq);

      // Validação de Alterações de Status
      ...
      if (statusDiffer) {
        ...
        recordsToUpdateMap.set(nrSeq, mappedRecord);
      }
    }

    // 5. Salvar histórico de relatos técnicos sem duplicar para TODAS as OS válidas
    console.log('[Sync Status OS] Processando histórico de relatos técnicos...');
    const validSequences = Array.from(validItemsMap.keys());
    ...
    if (validSequences.length > 0) {
      const { data: existingRelatos, error: relatoQueryError } = await supabase
        ...
        .in('nr_sequencia', validSequences)
      ...
      if (relatoQueryError) {
        ...
        for (const [nrSeq, item] of validItemsMap.entries()) {
          ...
          if (novoRelato !== '') {
            ...
            if (isDifferent) {
              ...
              // Se o histórico mudou, garante que a OS principal seja atualizada para notificar via Realtime
              if (!recordsToUpdateMap.has(nrSeq)) {
                const nrGrupoPlanej = item.NR_GRUPO_PLANEJ !== undefined && item.NR_GRUPO_PLANEJ !== null ? Number(item.NR_GRUPO_PLANEJ) : null;
                recordsToUpdateMap.set(nrSeq, {
                  nr_sequencia: nrSeq,
                  ds_grupo_des: item.DS_GRUPO_DES ? String(item.DS_GRUPO_DES).trim() : null,
                  nr_seq_localizacao: item.NR_SEQ_LOCALIZACAO ? Number(item.NR_SEQ_LOCALIZACAO) : null,
                  ds_localizacao: item.DS_LOCALIZACAO ? String(item.DS_LOCALIZACAO).trim() : null,
    ...
    const recordsToUpdate = Array.from(recordsToUpdateMap.values());

    console.log(`[Sync Status OS] Total de OS identificadas com alteração de status ou novo histórico: ${recordsToUpdate.length}`);

    // 6. Persistir novos relatos na tabela historico_ordem_servico em lotes de 500
    const chunkSize = 500;
    if (relatosToInsert.length > 0) {
      console.log(`[Sync Status OS] Inserindo ${relatosToInsert.length} novos relatos técnicos no histórico em lotes de ${chunkSize}...`);
      for (let i = 0; i < relatosToInsert.length; i += chunkSize) {
        const batchRelatos = relatosToInsert.slice(i, i + chunkSize);
        const { error: insertRelatoError } = await supabase
          .from('historico_ordem_servico')
          .insert(batchRelatos);

        if (insertRelatoError) {
          console.error('[Sync Status OS] Erro ao inserir lote de relatos na tabela historico_ordem_servico:', insertRelatoError.message);
        }
      }
    }
    ...
    if (recordsToUpdate.length === 0) {
      return new Response(
        JSON.stringify({
          success: true,
          message: 'Todas as Ordens de Serviço cadastradas estão com status e histórico atualizados.',
          updated: 0,
    ...
    // 7. Executar UPSERT com base em nr_sequencia na tabela public.ordem_servico em lotes de 500
    console.log(`[Sync Status OS] Realizando UPSERT de ${recordsToUpdate.length} ordens de serviço em lotes de ${chunkSize}...`);
    for (let i = 0; i < recordsToUpdate.length; i += chunkSize) {
      const batchRecords = recordsToUpdate.slice(i, i + chunkSize);
      const { error: upsertError } = await supabase
        .from('ordem_servico')
        .upsert(batchRecords, { onConflict: 'nr_sequencia' });
